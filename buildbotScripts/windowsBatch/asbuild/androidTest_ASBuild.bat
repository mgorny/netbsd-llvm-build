...
rm -rf %lldbDir%
call unzip -o %rootDir%\lldb-tests-* -d %lldbDir%\ || goto :error
...
call adb -s %deviceId% push %buildDir%\%bindir%\lldb-server %remoteDir%/ || goto :error
...
call %PYTHONHOME%\bin\python.exe %lldbDir%\packages\Python\lldbsuite\test\dotest.py ^
