...
screen -d -m adb -s "$deviceId" shell \
    env \
    TMPDIR="$remoteDir/tmp" \
    LLDB_DEBUGSERVER_LOG_FILE=server.log \
    LLDB_SERVER_LOG_CHANNELS="gdb-remote packets:lldb all" \
    "$remoteDir/lldb-server" platform --listen $listen_url --server
